...
const Renewals = () => {
  ...
  const fetchRenewal = async () => {
    ...
    {
      member_search_mobile &&
        member_search_appType &&
    ...
    try {
      ...
      if (response.data && !response.data.mobile.registered) {
        ...
        if (
          response.data.mobile.earlyBird === true ||
          response.data.mobile.earlyBird === 'true'
        ) {
          setEarlyBird(true);
        }
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      Surname: renewalData.Surname || '',
      Email: renewalData.Email || '',
      Mobile: renewalData.Mobile || '',
      Address: renewalData.Address || '',
      Suburb: renewalData.Suburb || '',
      PostCode: renewalData.PostCode || '',
      DateOfBirth: renewalData.DateOfBirth
        ? renewalData.DateOfBirth.substring(0, 10)
        : '',
      Gender: renewalData.Gender || '',
      membershipLevel: renewalData.membershipType || '',
      cardNumber: renewalData.CardNumber || '',
      expiryDate: renewalData.expiryDate
        ? renewalData.expiryDate.substring(0, 10).split('-').reverse().join('-')
        : '',
      cvv: '',
      nameOnCard: '',
      country: 'Australia',
            ...
            <div className="renewal-box">
              ...
              {member === 'new' && (
                <p className="sub-text">
                  Member does not exist please have member sign up as new
                  member.
                </p>
              )}

            ...
            {isAddingNew || isEditing ? (
              <Editor
                ...
                init={{
                  ...
                  //for removing general from image upload in editor
                  setup: (editor) => {
                    // helper: when a TinyMCE dialog node is added, keep only the "Upload" tab+panel
                    const keepOnlyUploadTab = (dialog) => {
                      try {
                        // nav items (tabs) and panels in the dialog
                        const navItems = Array.from(
                          dialog.querySelectorAll('.tox-dialog__body-nav-item')
                        );
                        const panels = Array.from(
                          dialog.querySelectorAll(
                            '.tox-dialog__body .tox-tab-panel'
                          )
                        );

                        // find the Upload tab index (case-insensitive)
                        const uploadIndex = navItems.findIndex((n) =>
                          /upload/i.test(n.textContent || '')
                        );

                        if (uploadIndex === -1) return; // nothing to do if no Upload tab

                        // remove all other nav items and panels, keep the upload one
                        navItems.forEach((n, i) => {
                          if (i !== uploadIndex) n.remove();
                        });
                        panels.forEach((p, i) => {
                          if (i !== uploadIndex) p.remove();
                        });

                        // ensure the remaining Upload tab is activated
                        const remainingNav = dialog.querySelector(
                          '.tox-dialog__body-nav-item'
                        );
                        if (remainingNav) remainingNav.click();
                      } catch (err) {
                        // harmless fail-safe
                        console.error('hide-image-tabs error', err);
                      }
                    };

                    // observe document for dialogs being added (robust vs timing issues)
                    const mo = new MutationObserver((mutations) => {
                      for (const m of mutations) {
                        for (const node of m.addedNodes) {
                          if (node && node.nodeType === 1) {
                            // TinyMCE dialog root has class 'tox-dialog'
                            if (node.matches && node.matches('.tox-dialog')) {
                              keepOnlyUploadTab(node);
                            } else {
                              // sometimes dialog is nested/inserted deeper; check descendants
                              const dlg =
                                node.querySelector &&
                                node.querySelector('.tox-dialog');
                              if (dlg) keepOnlyUploadTab(dlg);
                            }
                          }
                        }
                      }
                    ...
                    // clean up if editor is removed
                    editor.on('remove', () => mo.disconnect());
                  },
